...
func Ask(prompt string) (password string, err error) {
	return FAsk(os.Stdin, os.Stdout, prompt)
}
...
func FAsk(in io.Reader, out io.Writer, prompt string) (password string, err error) {
	if in != nil && prompt != "" {
		fmt.Fprint(out, prompt) // Display the prompt.
	}
	password, err = getPassword(in)
...
func readline(in io.Reader) (value string, err error) {
	var valb []byte
	var n int
	b := make([]byte, 1)
	for {
